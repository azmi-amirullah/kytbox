          ...
          <div>
            <div className='flex items-baseline gap-2.5'>
              <span className='text-3xl font-extrabold tracking-tight text-foreground'>
                {subscribers.length > totalSubscribers ? subscribers.length : totalSubscribers}
              </span>
          ...
          <div className='flex items-center justify-between gap-3'>
            <div>
              <div className='flex items-center gap-2'>
                <span className='text-sm font-bold text-foreground'>Download List</span>
              ...
              <table className='w-full text-left text-xs'>
                <thead className='bg-muted/40 text-muted-foreground font-medium uppercase tracking-wider border-b border-border/60'>
                  <tr>
                    ...
                    <th className='px-4 py-3 hidden md:table-cell'>Subscribed Date</th>
                    ...
                    <tr key={sub.id} className='hover:bg-muted/20 transition-colors'>
                      <td className='px-4 py-3 font-medium text-foreground'>
                        <div className='flex items-center gap-2'>
                          ...
                          <span className='truncate max-w-50 sm:max-w-xs'>{sub.email}</span>
                        </div>
                      </td>
                      ...
                      <td className='px-4 py-3 text-muted-foreground hidden md:table-cell whitespace-nowrap'>
                        <div className='flex items-center gap-1.5'>
                          ...
                          <span>{format(new Date(sub.created_at), 'MMM d, yyyy · HH:mm')}</span>
                        </div>
                      </td>

                      <td className='px-4 py-3 text-muted-foreground hidden sm:table-cell max-w-37.5 truncate'>
                        {sub.source_url ? (
                          <div className='flex items-center gap-1.5' title={sub.source_url}>
                            <LuGlobe className='w-3.5 h-3.5 text-muted-foreground/70 shrink-0' />
                            <span className='truncate'>{sub.source_url.replace(/^https?:\/\//, '')}</span>
                          </div>
                        ) : (
                          <span className='text-muted-foreground/50'>Direct Bio</span>
                        )}
                      </td>
