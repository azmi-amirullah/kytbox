...
| `budget_warning` | Monthly category spending reaches 80%–99% of budget limit | Budget Warning ⚠️ | `Food reached 85% of $500 budget` (localized) | `/cashflow/{cashflow_id}` | ⚠️ `LuTriangleAlert` (Amber) |
| `budget_exceeded` | Monthly category spending exceeds 100% of budget limit | Budget Exceeded 🔴 | `Food is over budget by $45.00` (localized) | `/cashflow/{cashflow_id}` | 🔴 `LuCircleAlert` (Red) |
